...
class ApiEmployee:
    ...
    def __init__(self):
        ...
        log.info("删除、更新、查询公共url模板：{}".format(self.url_delete))
    ...
    def api_put_employee(self):
        ...
        log.info("修改员工url：{} 修改员工数据：{} 请求header:{}".format(self.url_put.format(api.user_id),data, api.headers))
        # 2. 调用put方法 ->返回响应对象
        return requests.put(url=self.url_put.format(api.user_id), json=data, headers=api.headers)

    # 4. 查询员工
    def api_get_employee(self):
        log.info("查询员工url:{} 请求头：{}".format(self.url_get.format(api.user_id), api.headers))
    ...
    def api_delete_employee(self):
        log.info("删除员工url : {} 删除员工请求信息头：{}".format(self.url_get.format(api.user_id), api.headers))
        # 1. 调用delete方法 ->返回响应对象
